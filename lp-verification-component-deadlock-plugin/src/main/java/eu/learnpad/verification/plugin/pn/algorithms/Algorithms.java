...
public class Algorithms {
	...
	public static PetriNet generateReducedNet(PetriNet pn) throws Exception{
		/* Applica regole più complesse che generano una petri net minima (potrebbe renderne più difficile la lettura)
		 Regole:
		 - T1->P->T2 = posso togliere p ed unire le T (copiare tutti i next di T2 in T1 e i previous di T2 in T1 e rimuovere T2) solo se p ha un solo next ed un solo previous e (T2 ha un solo previous o T1 ha un solo next): questa modalità riduce di piu gli stati ma in questo caso bisogna anche controllare che non esista una connessione tra un previous di t2 e t1. Se esiste non si può applicare perchè andrebbe a eliminare deadlock: Es: in questo caso non si può ridurre: p0->t1->p->t2->p1, p0->t2
		 - P1->T->P2 = posso togliere T ed unire le P (copiare tutti i previous di P1 in P2 e i next di P1 in P2 e rimuovere P1) solo se T ha un solo next ed un solo previous e (P1 ha un solo next o P2 ha almeno un next): questa modalità riduce di piu gli stati ma in questo caso bisogna anche controllare che non esista una connessione tra un next di P1 ed un next di P2. Se già esiste non si può applicare perchè andrebbe ad eliminare deadlock: Es: in questo caso non si può ridurre: p0->t0->p1->t1, p0->t1
		 - per ogni T con un prev e un next controllare se ne esiste un altra con stesso num di prev e next e che abbia lo stesso prev e next. Se c'è toglierla.
		 - per ogni P con un prev e un next controllare se ne esiste un altro con stesso num di prev e next e che abbia lo stesso prev e next. Se c'è toglierlo.
		 poi ripetere
		 */
