...
public class MyGlimpseProbe_SimMonLearnPAd extends GlimpseAbstractProbe {
	...
	private void generateAndSendExample_GlimpseBaseEvents_StringPayload(String data, AbstractEvent event) throws UnknownHostException {
		...
		DebugMessages.ok();
		DebugMessages.line();
		message = new GlimpseBaseEventBPMN<String>(data, "aProbe", System.currentTimeMillis(), "eventForCP", false, "extraField", event);
		try {
	...
	public static void main(String[] args) throws UnknownHostException {
		...
		try {
			...
			aGenericProbe.generateAndSendExample_GlimpseBaseEvents_StringPayload("notifySimulationStart", event);
					
			Thread.sleep(2000);
			
			event = new ProcessStartEvent();
			aGenericProbe.generateAndSendExample_GlimpseBaseEvents_StringPayload("notifyProcessStart", event);
			
			Thread.sleep(2000);
			
			event = new TaskStartEvent();
			aGenericProbe.generateAndSendExample_GlimpseBaseEvents_StringPayload("notifyTaskStart", event);
			
			Thread.sleep(2000);
			
			event = new TaskFailedEvent();
			aGenericProbe.generateAndSendExample_GlimpseBaseEvents_StringPayload("notifyTaskFailed", event);
			...
			aGenericProbe.generateAndSendExample_GlimpseBaseEvents_StringPayload("notifyTaskEnd", event);
			
			Thread.sleep(2000);
			
			event = new ProcessEndEvent();
			aGenericProbe.generateAndSendExample_GlimpseBaseEvents_StringPayload("notifyProcessEnd", event);
			
			Thread.sleep(2000);
			
			event = new SessionScoreUpdateEvent();
			aGenericProbe.generateAndSendExample_GlimpseBaseEvents_StringPayload("notifySessionScoreUpdate", event);
			
			Thread.sleep(2000);
			
			event = new SimulationEndEvent();
			aGenericProbe.generateAndSendExample_GlimpseBaseEvents_StringPayload("notifySimulationEnd", event);
